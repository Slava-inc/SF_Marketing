...
class Function:
    ...
    async def create_diagram(self, call_back: CallbackQuery):
        ...
        path = FSInputFile(os.path.join(os.path.split(os.path.dirname(__file__))[0],'SF_Marketing/diagram.png'))
        answer = await self.bot.push_photo(call_back.message.chat.id, 'Диаграмма Ваших расходов',
                                           self.build_keyboard(button_done, 1),
                                          path)
        os.remove('diagram.png')
        return answer

    async def start_ai(self, call_back: CallbackQuery):
        client = Client("THUDM/CodeGeeX")
        result = client.predict(
            message="Привет! Напиши любой фианнсовый совет",
            api_name="/chat"
        )
        button_done = {'done_reminder_days': 'Готово ✅'}
        answer = await self.bot.push_photo(call_back.message.chat.id, result,
                                           self.build_keyboard(button_done, 1), self.bot.logo_goal_menu)
