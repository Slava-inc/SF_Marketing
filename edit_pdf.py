...
def outlay_dict_tinkoff(result: list) -> dict:
    dict_text_pdf = {}
    i = 1
    for item in result:
        if i == 2:
            dict_text_pdf['Наименование банка'] = change_char(item, '\n', ' ').split(' ХУТОРСКАЯ')[0]
        elif i == 3:
            dict_text_pdf['Номер и дата п/п:'] = change_char(item, '\n', ' ').split('Исх. № ')[1]
        elif i == 4:
            dict_text_pdf['Плательщик:'] = change_char(item, '\n', ' ').split('которой является ')[1]
        elif i == 4:
            dict_text_pdf['Дата и время операции:'] = change_char(item, '\n', ' ')
        elif i == 11:
            dict_text_pdf['Сумма в валюте операции:'] = change_char(item, '\n', ' ')
        elif i == 12:
            dict_text_pdf['Информация о получателе средств:'] = change_char(item, '\n', ' ')
        i += 1
    return dict_text_pdf

def receipt_dict_tinkoff(result: list) -> dict:
    dict_text_pdf = {}
    i = 1
    for item in result:
        if i == 2:
            ...
            dict_text_pdf['Сумма:'] = change_char(item, '\n', ' ').split(' i')[0]
        i += 1
    return dict_text_pdf

def get_tinkoff(result: list):
    if 'АО «ТБАНК»' in result[1]:
        dict_tinkoff = outlay_dict_tinkoff(result)
    else:
        dict_tinkoff = receipt_dict_tinkoff(result)
    return dict_tinkoff

def processing_pdf(path_pdf: str):
    # Находим путь к PDF
    pdf_path = path_pdf
    ...
    return text_per_page['Page_0'][4]


dict_text = get_tinkoff(processing_pdf('receipt_04.10.2024.pdf'))
